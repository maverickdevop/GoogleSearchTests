...
class BasePage:
    ...
    @allure.title("Метод клика по кнопкам из списка")
    def do_click_keyboard(self, button_names):
        for button_name in button_names:
            if button_name in GooglePage.BUTTONS:
                by_locator = GooglePage.BUTTONS[button_name]
                element = self.wait.until(EC.element_to_be_clickable(by_locator))
                element.click()
            else:
                print(f"Button '{button_name}' not found in GooglePage.BUTTONS.")
    ...
    def assert_text_in_list(self, by_locator, text):
        ...
        print("Список текста:", list_links)

        for links in list_links:
            assert text in links

    @allure.title("Метод проверки, что хотя бы 1 слово есть списке")
    def assert_word_in_list(self, by_locator, text):
        list_elements = self.wait.until(EC.presence_of_all_elements_located(by_locator))
        list_links = []
        for item in list_elements:
            list_links.append(item.text)
        print("Список текста:", list_links)
        assert text in list_links
    ...
    @allure.title("Метод переключения на предыдущее окно браузера")
    def upload_data(self, by_locator):
        element = self.wait.until(EC.presence_of_element_located(by_locator))
        element.click()

        relative_image_path = Data.IMG_PATH
        project_dir = os.path.dirname(os.path.abspath(__file__))
        image_path = os.path.join(project_dir, relative_image_path)
